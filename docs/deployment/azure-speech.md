...
主控板的字幕區會把目前語音語言視為即時字幕語言，並從預覽列表排除相同語言。
例如語音語言為國語時，即時區顯示繁體中文，預覽區只顯示其他已選字幕輸出語言；
語音語言為英語時，預覽區不再顯示英文。
...
- 即時字幕區：顯示目前講者語言，只使用 Speech recognizing 階段回傳的非空中間辨識結果。
  即使 Speech Translation 回傳最終辨識結果，Portal 也不會把 final transcript 補進即時字幕區。
  若 recognizing 結果是空字串，Portal 會忽略該次更新並保留上一個有效的即時結果。
- 翻譯預覽區：排除與目前語音語言相同的字幕輸出語言。
  若 Speech SDK 在 recognizing 階段提供中間翻譯，Portal 會優先顯示中間翻譯；
  若沒有中間翻譯，則顯示最後一次可用的最終翻譯結果。
...
若字幕預覽中央區有輸入工作階段標題，資料夾名稱會加上空格與清理後的標題；
檔名依字幕輸出語言代碼命名，例如：
