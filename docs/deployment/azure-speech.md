...
Portal 目前只實作來源語音辨識，尚未啟用翻譯字幕與 Relay 發布。開啟收音只會啟動主控板選定音訊來源與音量監測；使用者點擊「開始字幕」後，Portal 才會使用同一個選定音訊來源建立 Azure Speech recognizer。若沒有選定音訊來源，來源語音辨識不會 fallback 到系統預設麥克風。

字幕預覽旁的狀態標籤代表 Speech 來源語音辨識狀態：

- `等待語音`：Speech recognizer 尚未啟動或已停止。
- `聆聽中`：Speech recognizer 已啟動，正在等待可辨識語音。
- `辨識中`：Speech recognizer 已收到中間辨識結果。
- `辨識失敗`：Speech recognizer 回報取消或錯誤。

使用者點擊「停止字幕」時，Portal 會停止 Speech recognizer，但保留字幕預覽中最後顯示的辨識文字與字幕事件計數，避免操作停止時清空現場畫面。

Portal 主控板的工作階段區塊會依 `speech.authorizationStatus` 顯示 Speech 授權狀態：
