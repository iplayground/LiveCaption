...
`VIEWER_ACCESS_CODE_REQUIRED=false` 是活動模式規格，代表任何可呼叫 Relay negotiate endpoint 的觀眾端都可取得短效 viewer URL。這符合公開活動字幕的產品定位，但 URL token 仍不得被 logs 或監控輸出保存，且觀眾端 group 權限必須維持 receive-only。
