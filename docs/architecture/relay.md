...
7. Portal 主控板可用相同 receive-only negotiate 流程作為操作端觀察視角，顯示 Relay 發布到 Azure Web PubSub 後再收到的字幕；Portal 仍不得取得 Web PubSub 發布權限。
...
Relay 會同時發布到 base group 與 track group。多軌活動時，觀眾端與 Portal 觀察端應在 negotiate body 帶入 `trackNumber`，只加入指定軌道 group。
