...
4. Portal 依 final 字幕品質模式選擇字幕來源：快速使用 Azure Speech final，精準使用 Azure Speech final 作為時間 anchor 與輔助候選文字，並以 Azure OpenAI transcription deployment 產生原始語言 draft，再由 Azure OpenAI text model 以 OpenAI transcription 為主要候選、Azure Speech final 為輔助候選產生校正後原文與翻譯結果。
