...
final 字幕。Portal 使用 realtime transcription final 作為精準字幕的原文、起訖時間與
句段邊界，並讓 realtime translation 結果跟隨該 transcription 句段補齊其他輸出語言。
精準模式輸出 `zh-Hant` 時，不論內容來自國語 transcription 原文或其他語言 translation，
Portal 都必須要求 Azure OpenAI 使用台灣繁體中文，並在寫入字幕事件前保底正規化為
台灣繁體中文，避免模型回傳簡體中文。
...
translation WebSocket。transcription session 依目前語音輸入語言產生原文 final 與時間碼；
translation session 依非原始語言的字幕輸出語言各自產生翻譯。Portal 收到 transcription
final 時建立一筆 `accurate` 字幕事件，並短暫等待 translation buffer 取得同一區段可用的
翻譯；若某個輸出語言尚未產生翻譯，該語言不會出現在該筆精準字幕的 Relay payload 或
`accurate/` SRT。`zh-Hant` 的 transcription 原文與 translation 結果都需在 Portal 端正規化為
台灣繁體中文。Portal 不會用 Azure Speech final 補入精準模式的原始語言。
...
- `Portal/LiveCaptionPortal/Speech/`：語音輸入語言、字幕輸出語言、Azure Speech 設定、Speech Translation 與 Azure OpenAI realtime transcription / translation。
