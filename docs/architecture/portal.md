...
Portal 送往 Relay 的字幕事件可同時包含快速與精準結果，Relay 依 `captionMode` 發布到不同 Web PubSub group，讓觀眾端可選擇接收 `fast` 或 `accurate`。
...
字幕輸出語言會建立一條 target language session，包含與語音輸入相同的原始語言輸出；
因此精準模式發布到 Relay 的原始語言字幕也應使用 Azure OpenAI 結果。Portal 目前在
Azure Speech final 事件發生時擷取 OpenAI transcript buffer 作為該句的精準字幕候選；
這是可運作的第一版對齊策略，後續需改進為以 OpenAI output item / audio boundary
對齊 Azure Speech final 分句。
