...
上方字幕預覽可在主畫面內嵌顯示或獨立視窗顯示。主畫面內嵌預覽維持 Azure Speech 來源，用於低延遲現場截取。獨立視窗預覽提供 `Speech` 與 `OpenAI` 字幕來源選項：`Speech` 顯示 Azure Speech 即時 recognizing 與 final 結果；`OpenAI` 只顯示 Azure OpenAI final 字幕，不顯示 partial 或 interim 文字。此選項只影響操作端投影截取預覽，不改變 Relay 發布、SRT 輸出或字幕品質模式的資料邊界。
...
匯出檔只包含已勾選的區塊。詞彙提示在環境設定檔中只保存文字陣列，不保存 Portal 內部 UI 使用的 UUID；匯入時會依文字重新建立本機 `SpeechPhraseHint`。Relay 匯出只包含 Relay URL，不包含會議室名稱與軌數，避免把同一場次或同一軌道的操作端狀態誤帶到其他電腦。
