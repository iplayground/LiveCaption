...
7. Portal 發布 Relay 字幕事件時依模式分開送出；快速事件只包含 `fast`，精準事件只包含
   ...
   Relay 發布。
8. Portal 結束字幕時，SRT 輸出也依模式分開保存。Azure Speech final 字幕寫入 `fast/`
   子資料夾，Azure OpenAI final 字幕寫入 `accurate/` 子資料夾；未產生內容的模式不寫空
   SRT 檔。
9. Portal 使用本機設定保存 Azure OpenAI API key；不得寫入範例文件、logs 或 Relay
   設定。Azure OpenAI realtime 授權方式若後續改為短效 token 或其他流程，仍不得混入
   Relay 字幕轉發流程。
10. 若精準模式失敗、逾時或尚未取得 final 結果，產品行為必須保守：可回退使用 Azure
