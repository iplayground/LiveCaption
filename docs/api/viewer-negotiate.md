...
當 Relay 設定 `VIEWER_ACCESS_CODE_REQUIRED=true` 時，觀眾端 App 必須在 `X-LiveCaption-Viewer-Access-Code` header 帶入 Portal 透過 `HEAD /api/caption-events` 取得並顯示的 access code。Request body 不需要內容。

Relay 驗證通過後會回傳可接收 `caption-live` group 訊息的短效 WebSocket URL。
...
| `group` | Relay 發布字幕的 group 名稱，第一版為 `caption-live`。 |
| `expiresAt` | URL 內 token 的預期到期時間。觀眾端 App 應在到期前或斷線後重新 negotiate。 |

## 權限

Relay 產生的觀眾端 URL 只會讓 client 連線後加入 `caption-live` group，不會授予任何 `webpubsub.sendToGroup` 權限。
