...
目前專案已建立 Portal macOS App 的主畫面與 Relay 後端。
Portal 可產生字幕事件並送往 Relay；Relay 可驗證 Portal 請求、發布字幕到 Azure Web PubSub，
並提供觀眾端 App 取得 receive-only Web PubSub 連線 URL 的 negotiate API。
...
- 主畫面包含上方字幕預覽、工作階段、音訊輸入、字幕檔案、語音分析預覽、
  Speech 設定、Relay 設定、最近狀態與底部可展開事件紀錄。
...
- 字幕檔案區塊可設定 SRT 輸出根目錄，路徑以尾段為主顯示，
  ...
  並在工作階段區塊顯示檔案權限狀態。
- App 開啟後，中央語音分析預覽會依每個字幕框的語言顯示歡迎文字。
  點擊「開始字幕」後，中央即時區顯示目前語音語言的 recognizing 非空結果；
  收到 final 時也會顯示 final，但同一筆較舊的 recognizing 不會覆蓋 final。
  若 Speech SDK 傳回空字串，Portal 不會更新畫面或清空既有內容。
- 語音分析預覽中央區可輸入本次工作階段標題。標題輸入框不會在 App 啟動時自動取得焦點；
  只有使用者點擊輸入框時才進入焦點模式，
...
- 中央藍色預覽區會排除與即時語音相同的語言，且一律顯示 final 翻譯，
  不使用 recognizing 階段的中間翻譯。
  字幕預覽更新是 Portal 主執行緒的最高優先 UI 工作，Relay 發布、SRT 累積、
  事件計數與事件紀錄會延後處理，避免阻塞即時字幕畫面。
- 上方字幕預覽是給投影導播截取的白底區塊。右側 `字幕預覽設定` 可調整語言、
  寬高、字體、字體大小、行距、左右間距與疊加時保留的字幕筆數；
  開始字幕後到停止前，這些設定不可變動。設定浮窗會避開上方字幕預覽區。
  上方預覽文字固定置底、置左，字體大小為強制值，不會依空間自動縮放。
- 上方字幕預覽可選擇取代模式或疊加模式。疊加模式會在既有內容下一行 append，
  並保留最近幾筆 final 字幕；若上方預覽語言和語音輸入對應語言相同，才顯示 recognizing。
  若兩者不同，上方預覽只顯示該字幕語言的 final history，不顯示 recognizing。
  設定中的「清空」會清空上方預覽 history 並保持空白，不會自動填入歡迎文字；
  「填入」會把目前語音分析中的最後一筆 final 字幕加入上方預覽 history。
- Relay 設定由 Portal 右側 Relay 區塊開啟，
  包含 Relay URL、可留空的會議室名稱與整數軌道值。
  測試 Relay 時，Portal 會先儲存並正規化設定，再以 `HEAD /api/caption-events`
  驗證簽章並取得觀眾端 access code，不會發布測試字幕；
  若 Relay 發布字幕事件失敗，
...
- `Portal/LiveCaptionPortal/UI/`：Header、側欄、字幕預覽、語音分析預覽、Speech 與 Relay 設定 sheet、
...
Relay 是後端服務。它會接收來自 Portal 的字幕事件，驗證事件格式與請求簽章，並透過 Azure Web PubSub publisher adapter 發布字幕。觀眾端 App 可透過 Relay 取得短效 receive-only Web PubSub client access URL，直接連線 Azure Web PubSub 接收公開字幕；是否必須帶 Portal 顯示的 access code 由 Relay 的 `VIEWER_ACCESS_CODE_REQUIRED` 設定控制。
