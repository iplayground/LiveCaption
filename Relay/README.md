...
目前 Relay 已建立 Python 3.13 Azure Functions 骨架、字幕事件 validator、Speech key HMAC 請求驗證、Azure Web PubSub 基礎設施設定與 publisher adapter。HTTP handler 驗證成功後會發布字幕 payload 到設定的 Web PubSub group。正式 Azure Relay 需以 GitHub Actions 部署包含此變更的 commit 後，才會執行新版 publisher 程式。
...
- `src/relay/webpubsub.py`：Azure Web PubSub publisher adapter。
- `tests/`：pytest 測試。
