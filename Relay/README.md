...
目前 Relay 已建立 Python 3.13 Azure Functions 骨架、字幕事件 validator、Speech key HMAC 請求驗證、Azure Web PubSub 基礎設施設定、publisher adapter 與觀眾端 receive-only negotiate endpoint。Portal 以 `HEAD /api/caption-events` 測試 Relay 並取得觀眾端 access code，正式字幕以 `POST /api/caption-events` 發布到 Web PubSub group；觀眾端 App 透過 Relay 換取短效 Web PubSub 連線 URL，當 `VIEWER_ACCESS_CODE_REQUIRED=true` 時需帶 Portal 顯示的 access code。
...
`AZURE_WEBPUBSUB_GROUP_NAME` 定位字幕發布目標，並透過
`VIEWER_ACCESS_CODE_REQUIRED` 控制觀眾端 negotiate 是否必須帶 Portal 顯示的
access code。正式 Azure Functions 由 Bicep 寫入這些非機密設定；Portal 不會把
Speech key 或 Azure Web PubSub 發布細節直接傳給 Relay，而是用本機 Speech key
對 request body 產生 HMAC 簽章。
...
觀眾端連線 API 記錄於 [../docs/api/viewer-negotiate.md](../docs/api/viewer-negotiate.md)。
